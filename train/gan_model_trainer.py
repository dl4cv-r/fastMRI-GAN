...
class GANModelTrainer:
    ...
    def _train_step(self, inputs, targets, extra_params):
        ...
        target_real = self.target_real.expand_as(pred_real)  # Expand to same shape as discriminator output.
        loss_real = self.loss_funcs['gan_loss_func'](pred_real, target_real)

        # This part is used later when training the generator.
        recons = self.generator(inputs)

        # Gradients from the generator should not influence the discriminator.
        pred_fake = self.discriminator(recons.detach())  # Generated fake image going through discriminator.
        target_fake = self.target_fake.expand_as(pred_fake)  # Expand to same shape as discriminator output.
        loss_fake = self.loss_funcs['gan_loss_func'](pred_fake, target_fake)

        disc_loss = (loss_real + loss_fake) * 0.5  # Average of real and fake.
        ...
        pred_fake = self.discriminator(recons)  # This is repeated since the discriminator was just updated.
        # This maximizes log(D(G(x)), which is just log(1-(1-D(G(x)))).
        gen_loss = self.loss_funcs['gan_loss_func'](pred_fake, target_real)  # This is probably right, I think...
        ...
        step_loss_components = {'loss_real': loss_real, 'loss_fake': loss_fake, 'disc_loss': disc_loss,
                                'gen_loss': gen_loss, 'recon_loss': recon_loss, 'total_gen_loss': total_gen_loss}
